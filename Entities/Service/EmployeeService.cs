...
namespace EmployeeTurim.Service.Service
{
...

    {
        ...
        public async Task<List<Employee>> AddMultipleEmployees(List<Employee> employees)
        {
            ...
            {
                Employee employeeInDB = await _employeeRepository.GetEmployeeByRegistrationNumberAndName(employee.RegNumber, employee.Name);
                if (employeeInDB != null)
                {
                    throw new Exception("Funcionario " + employee.Name + " Matricula: " + employee.RegNumber + " já cadastrado");
                }
            }
